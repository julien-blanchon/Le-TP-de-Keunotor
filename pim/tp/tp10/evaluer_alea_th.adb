...
-- Évaluer la qualité du générateur aléatoire avec TH.
procedure Evaluer_Alea_TH is
	...
	is
		...
		--* Dans TH qui soit suffisament flexible pour permettre se genre de traitement.
	...
	begin
		-- Initialiser
		Initialiser(Ma_TH);
		-- Remplir la TH avec les fréquences
		for I in 1..Taille loop
			Get_Random_Number(Nombre);
			if Cle_Presente(Ma_TH, Nombre) then
				Enregistrer(Ma_TH, Nombre, La_Donnee(Ma_TH, Nombre)+1);
			else
				Enregistrer(Ma_TH, Nombre, 1);
			end if;
		end loop;
		-- Calcul des fréquences minimale et maximale
		Min := -1;
		Max := -1;
		Determiner_MinMax_Pour_Chaque(Ma_TH);
		Vider(Ma_TH);
