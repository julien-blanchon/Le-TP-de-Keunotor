...
generic
	...
	-- Relation d'ordre totale des clés :
...
package ABR is
  ...
  procedure Decrocher_Min (Sda : in out T_ABR; Min : out T_ABR); --with
		--Pre => Sda /= Null,
		--Post => Min /= Null
		--	and Taille (Min) = 1		-- une feuille
		--	and Taille (Sda) = Taille (Sda)'Old - 1	-- un décroché
		--	and not (Sda'Old.All.Cle < Min.All.Cle);	-- clé plus petite !
